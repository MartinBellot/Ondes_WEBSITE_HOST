  ...
  Future<void> _deploy() async {
    ...
    if (mounted) {
      ...
      if (ok) {
        ...
        _name.clear(); _image.clear(); _hPort.clear();
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(context.read<DockerProvider>().error ?? 'Deploy failed'),
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Flexible(
                          ...
                          child: _DeployForm(
                            formKey: _formKey,
                            name: _name, image: _image,
                            hPort: _hPort, cPort: _cPort,
                            volHost: _volHost, volCont: _volCont,
...
class _DeployForm extends StatelessWidget {
  ...
  const _DeployForm({
    ...
    required this.hPort, required this.cPort,
    required this.volHost, required this.volCont,
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 14),
            Row(children: [
              Expanded(child: _Field('Volume Host Path',      volHost, hint: '/data/app')),
              const SizedBox(width: 12),
              Expanded(child: _Field('Volume Container Path', volCont, hint: '/app/data')),
            ]),
        ...
        children: [
          ...
          if (docker.isLoading)
            ...
            const Padding(
              padding: EdgeInsets.all(32),
              child: Center(
                child: Text('No containers', style: TextStyle(color: AppColors.textMuted)),
              ),
            )
          else
            ...docker.containers.map((c) => _DockerRow(container: c, docker: docker)),
        ],
      ),
    );
  }
      ...
      child: Row(
        children: [
          ...
          _MiniBtn(
            ...
            onTap: () => docker.performAction(container['id'], isRunning ? 'stop' : 'start'),
          ),
  ...
  Widget build(BuildContext context) => MouseRegion(
        ...
        child: GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: _hov ? widget.color.withOpacity(0.15) : AppColors.surfaceVariant,
