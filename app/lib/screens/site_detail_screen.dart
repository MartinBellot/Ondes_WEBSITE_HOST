...
class _SiteDetailScreenState extends State<SiteDetailScreen>
  ...
  Future<void> _refreshSite() async {
    ...
    if (mounted) setState(() => _site = Map<String, dynamic>.from(updated as Map));
  }
              ...
              children: [
                ...
                Expanded(
                  child: TabBarView(
                    controller: _tabCtrl,
                    children: [
                      _GeneralTab(site: _site, onSaved: (updated) {
                        setState(() => _site = updated);
                      }),
                      _GitHubTab(site: _site, onSaved: (updated) {
                        setState(() => _site = updated);
                      }),
...
class _GeneralTabState extends State<_GeneralTab> {
  ...
  @override
  void dispose() { _name.dispose(); _domain.dispose(); super.dispose(); }

  Future<void> _save() async {
    setState(() => _saving = true);
    final ok = await context.read<SitesProvider>().updateSite(widget.site['id'] as int, {
    ...
    if (mounted) {
      ...
      if (ok) {
        widget.onSaved({...widget.site, 'name': _name.text.trim(), 'domain': _domain.text.trim(), 'site_type': _type});
    ...
    return _TabScaffold(
      children: [
        ...
        _Select(
          label: 'Type de site',
          value: _type,
          items: const {'web': 'Web / Frontend', 'api': 'API / Backend', 'fullstack': 'Fullstack'},
...
class _GitHubTabState extends State<_GitHubTab> {
  ...
  void initState() {
    ...
    _selectedRepo   = widget.site['github_repo']?.isNotEmpty == true ? widget.site['github_repo'] : null;
  ...
  Future<void> _verify() async {
    ...
    final ok = await context.read<GitHubProvider>().connect(_tokenCtrl.text.trim());
    if (ok && mounted) {
      context.read<GitHubProvider>().fetchRepos(_tokenCtrl.text.trim());
      setState(() { _tokenVerified = true; _verifying = false; });
    } else if (mounted) {
  ...
  Future<void> _saveAndDeploy() async {
    ...
    final result = await context.read<SitesProvider>().deploySite(widget.site['id'] as int);
    if (mounted) {
        ...
        if (gh.isConnected) ...[
          ...
          if (gh.isLoadingRepos)
            ...
            _RepoSelector(
              ...
              onSelected: (repo) {
                ...
                context.read<GitHubProvider>().fetchBranches(_tokenCtrl.text.trim(), repo);
              },
            ),
          if (_selectedRepo != null) ...[
            ...
            if (gh.isLoadingBranches)
              ...
              Wrap(
                spacing: 8,
                children: gh.branches.map((b) => _BranchChip(
                  label: b,
...
class _RepoSelector extends StatefulWidget {
  ...
  const _RepoSelector({required this.repos, required this.selectedRepo, required this.onSelected});
...
class _RepoTile extends StatefulWidget {
  ...
  const _RepoTile({required this.repo, required this.isSelected, required this.onTap});
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            color: widget.isSelected
                ...
                : _hov ? AppColors.surfaceVariant : Colors.transparent,
...
class _BranchChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            color: selected ? AppColors.accent.withOpacity(0.15) : AppColors.surfaceVariant,
...
class _DeployButton extends StatelessWidget {
  ...
  const _DeployButton({required this.repo, required this.branch, required this.deploying, required this.onDeploy});
          ...
          children: [
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('Prêt à déployer', style: TextStyle(color: AppColors.textPrimary, fontWeight: FontWeight.w500)),
...
class _HostingTabState extends State<_HostingTab> {
  ...
  void initState() {
    super.initState();
    _webCont = TextEditingController(text: widget.site['web_container_name'] ?? '');
    _webPort = TextEditingController(text: widget.site['web_port']?.toString() ?? '');
    _apiCont = TextEditingController(text: widget.site['api_container_name'] ?? '');
    _apiPort = TextEditingController(text: widget.site['api_port']?.toString() ?? '');
  }
  ...
  Widget build(BuildContext context) {
    return _TabScaffold(
      children: [
        ...
        const SizedBox(height: 4),
        const Text('Conteneur Docker servant le frontend statique ou l\'application web.',
            style: TextStyle(color: AppColors.textMuted, fontSize: 13)),
        const SizedBox(height: 14),
        _FormRow(children: [
...
class _DomainSslTabState extends State<_DomainSslTab> {
  ...
  @override
  void dispose() { _domainCtrl.dispose(); _emailCtrl.dispose(); super.dispose(); }
  ...
  Future<void> _requestCert() async {
    ...
    setState(() { _requesting = true; _certResult = null; });
    ...
    return _TabScaffold(
      children: [
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 14),
              _DnsRow(type: 'A', name: domain.isNotEmpty ? domain : '@', value: 'IP_DE_VOTRE_SERVEUR'),
          ...
          children: [
            ...
            if (ssl)
              Container(
                ...
                child: const Text('Actif', style: TextStyle(color: AppColors.accentGreen, fontSize: 11, fontWeight: FontWeight.w600)),
              ),
          ],
        ),
        const SizedBox(height: 4),
        ...
        ElevatedButton.icon(
          ...
          label: Text(_requesting ? 'En cours…' : ssl ? 'Renouveler le certificat' : 'Obtenir un certificat'),
        ),
        if (_certResult != null) ...[
          const SizedBox(height: 16),
          Container(
            ...
            child: Text(
              _certResult!,
              style: TextStyle(
                color: _certResult!.startsWith('✓') ? AppColors.accentGreen : AppColors.accentRed,
                fontSize: 13,
              ),
            ),
          ),
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Text(widget.type, style: const TextStyle(color: AppColors.accent, fontSize: 11, fontWeight: FontWeight.w700)),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Text(widget.name, style: GoogleFonts.jetBrainsMono(color: AppColors.textPrimary, fontSize: 13)),
          ),
          Text('→', style: const TextStyle(color: AppColors.textMuted)),
          const SizedBox(width: 12),
          Expanded(
            child: Text(widget.value, style: GoogleFonts.jetBrainsMono(color: AppColors.textSecondary, fontSize: 13)),
          ),
          GestureDetector(
            onTap: () async {
...
class _NginxTabState extends State<_NginxTab> {
  ...
  Future<void> _preview() async {
    setState(() { _loading = true; _config = null; });
    final cfg = await context.read<SitesProvider>().nginxPreview(widget.site['id'] as int);
    if (mounted) setState(() { _loading = false; _config = cfg; });
  }

  Future<void> _apply() async {
    setState(() => _applying = true);
    final result = await context.read<SitesProvider>().applyNginx(widget.site['id'] as int);
    if (mounted) {
      setState(() => _applying = false);
      if (result['status'] == 'success') {
        _snack(context, 'Configuration NGINX appliquée et rechargée', success: true);
      } else {
      ...
      children: [
        ...
        if (_config != null) ...[
          ...
          Row(
            ...
            children: [
              const Text('nginx.conf', style: TextStyle(color: AppColors.textMuted, fontSize: 12)),
              GestureDetector(
                onTap: () => Clipboard.setData(ClipboardData(text: _config!)),
                child: const Row(children: [
                  Icon(Icons.copy_outlined, size: 13, color: AppColors.textMuted),
                  SizedBox(width: 4),
                  Text('Copier', style: TextStyle(color: AppColors.textMuted, fontSize: 12)),
                ]),
              ),
            ],
          ),
          ...
          Container(
            ...
            child: SelectableText(
              _config!,
              style: GoogleFonts.jetBrainsMono(color: const Color(0xFF22C55E), fontSize: 13, height: 1.6),
            ),
          ),
        ],
      ],
